...
import com.jl.util.DateFormatUtil;
...
public class DwdBaseLog {
    ...
    public static void main(String[] args) {
        ...
        DataStreamSource<String> logStr = KafkaUtil.getKafkaSource(env, "topic_log", "topic_01");

        //log：str --> obj
        SingleOutputStreamOperator<JSONObject> etl = etl(logStr);
        etl.print("=----------------------");
        SingleOutputStreamOperator<JSONObject> fix_db = fixedNewAndOld(etl);
        Map<String, DataStream<String>> stringDataStreamMap = splitStream(fix_db);
        writeToKafka(stringDataStreamMap);
...
//        jsonObjDS.print("标准的json:");
        ...
        dirtyDS.addSink(KafkaUtil.getKafkaSink("dirty_data"));
